 ...
 */
export function resolveConfig(userConfig: Partial<OkfConfig>): OkfConfig {
  return {
    ...DEFAULT_CONFIG,
    ...userConfig,
  };
 ...
 */
export function validateConfig(config: OkfConfig): string[] {
  const errors: string[] = [];
  ...
  if (
    typeof config.minRecallScore === "number" &&
  ...
  if (
    typeof config.minMatchedTokens === "number" &&
