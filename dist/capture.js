 ...
 */
export function detectKeywordTrigger(userMessage) {
 ...
 */
export function analyzeForAutoCapture(userMessage, assistantResponse, config) {
    // Gate 1: Feature flag
    ...
    const userSignalMatch = AUTO_CAPTURE_SIGNALS.userSignals.some((pattern) => pattern.test(userMessage));
    if (!userSignalMatch) {
        return {
            shouldCapture: false,
    ...
    const assistantSignalMatch = AUTO_CAPTURE_SIGNALS.assistantSignals.some((pattern) => pattern.test(assistantResponse));
    ...
    // Build suggestion
    const suggestion = buildCaptureSuggestion(knowledgeType, userMessage, assistantResponse);
    return {
        shouldCapture: true,
 ...
 */
function detectKnowledgeType(userMessage, assistantResponse) {
 ...
 */
function containsGarbageSignals(response) {
 ...
 */
function buildCaptureSuggestion(knowledgeType, userMessage, _assistantResponse) {
    const typeMap = {
